...
const PROVIDER_ENV_VARS: Record<string, string> = {
  ...
  'apexyy-claude': 'APEXYY_CLAUDE_KEY',
  'apexyy-codex': 'APEXYY_CODEX_KEY',
};
...
const PROVIDER_DEFAULT_MODELS: Record<string, string> = {
  ...
  'apexyy-claude': 'apexyy-claude/claude-opus-4-6',
  'apexyy-codex': 'apexyy-codex/gpt-5.2',
};
...
const PROVIDER_CONFIGS: Record<string, { baseUrl: string; api: string; apiKeyEnv: string }> = {
  ...
  'apexyy-claude': {
    ...
    apiKeyEnv: 'APEXYY_CLAUDE_KEY',
  },
  'apexyy-codex': {
    baseUrl: 'https://yunyi.rdzhvip.com/codex',
    api: 'openai-responses',
    apiKeyEnv: 'APEXYY_CODEX_KEY',
  },
};

import { APEXYY_NODES, getClaudeBaseUrl, getCodexBaseUrl, type ApexYYNode } from './apexyy-nodes';

/**
 * Update the OpenClaw config to use the given provider and model
 * Writes to ~/.openclaw/openclaw.json
 * @param provider - Provider ID (e.g., 'apexyy-claude', 'anthropic')
 * @param apexyyNodeId - Optional ApexYY node ID (1-6) for apexyy-* providers
 */
export function setOpenClawDefaultModel(provider: string, apexyyNodeId?: number): void {
  ...
  if (providerCfg) {
    ...
    // For ApexYY providers, resolve baseUrl from selected node
    let resolvedBaseUrl = providerCfg.baseUrl;
    if (provider.startsWith('apexyy-') && apexyyNodeId) {
      const node = APEXYY_NODES.find(n => n.id === apexyyNodeId);
      if (node) {
        resolvedBaseUrl = provider === 'apexyy-claude' ? getClaudeBaseUrl(node) : getCodexBaseUrl(node);
      }
    }
    
    // Always update for apexyy providers (node may have changed), otherwise only set if not configured
    if (provider.startsWith('apexyy-') || !providers[provider]) {
      ...
      // ApexYY Claude needs special config
      if (provider === 'apexyy-claude') {
        providerEntry.headers = {};
        providerEntry.authHeader = false;
      }
      // ApexYY Codex needs explicit model declarations
      if (provider === 'apexyy-codex') {
      ...
      console.log(`Configured models.providers.${provider} with baseUrl=${resolvedBaseUrl}`);
    }
